...
const SYSTEM_PROMPT =
  ...
  "For each item in those two lists, the description is EXACTLY TWO short sentences (EXCEPT when the detected item is media — media reks are EXACTLY THREE, per the media clause below).\n" +
  ...
  "WHEN the detected item is media (movies, tv, books) — and ONLY then — the description is EXACTLY THREE sentences: story first, placement last. Media anchor and media reks share one grammar — the anchor places against its CATEGORY, the reks place against the ANCHOR. THREE sentences is media reks only: non-media items keep the two-sentence comparative-first order above, unchanged.\n" +
  ...
  "Media sentence 3 — the comparative placement against the snapped item: a bare comparative on tone/pace/scope. All comparative rules above carry over unchanged — the referent is implicit, never echo the anchor's name, at most ONE 'than the one you snapped' per set.\n" +
  "- RIGHT: 'A small-town notary discovers her signature stamp legally binds anything — starting with her landlord's eviction notice. Half the county lines up outside her office, and the state bar sends someone to take the stamp. Faster and funnier than a straight courtroom drama.'\n" +
...
const DIFFERENTIATOR_VOICE =
  "DIFFERENTIATOR RULE — the description is EXACTLY TWO short sentences (EXCEPT when the detected item is media — media reks are EXACTLY THREE, per the media clause below).\n" +
