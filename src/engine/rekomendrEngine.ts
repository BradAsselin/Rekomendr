...
${backfill ? `] }` : "]"}
Rules:
- For Movies, TV Shows, and Books: short is exactly three sentences, one job each — EQUAL care on all three; no sentence is the garnish.
- THE PRIME RULE, all three sentences: TELL THE STORY — never a summary OF the story. The blurb tells me the story; it never tells me about it. The premise's own specifics — numbers, deadlines, objects, places — appear in the sentences themselves; if the premise has a number or a named rule, it appears VERBATIM ('45 days', 'chosen the lobster' — never 'a time limit', 'an animal of their choosing').
- RIGHT vs WRONG (same fictional premise told twice — the story vs a summary of it):
  - RIGHT: 'A parking-garage attendant finds a sedan on level three with a ticket stub dated ten years ago — and 72 hours to return it before the garage's annual audit.'
